...
- **Post:** `docs/blog/posts/2026-09-07-idempotency-across-a-chain-of-microservices.md`
- **Category:** Design
- **Idea:** The key travels with the request all the way down, so one intent means one effect.
...
Subject: Three service housings of decreasing size arranged front to back along one continuous rail. A single small token block rides the rail and appears once at each housing, identical every time. At the far end exactly one effect disc is stamped into the base plane, and beside it three empty depressions of the same size. The token is the terracotta object.
