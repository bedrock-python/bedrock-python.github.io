# Kafka in a Python service: producers, consumers and operations

- **Post:** `docs/blog/posts/2026-09-13-kafka-in-python-services.md`
- **Category:** Tutorials
- **Idea:** Each default checked against a real broker, not against the documentation.
...
Subject: A vertical stack of thin plates resting on short standoffs, like a checklist made of physical layers. A measuring arm reaches from the stack across to an append-only log trough standing on the same base plane, touching it directly. On the opposite side, a closed book-like block lies flat with nothing connected to it. The measuring arm is terracotta.
