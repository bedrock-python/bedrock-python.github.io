# PostgreSQL partition maintenance: creation, archiving and deletion

- **Post:** `docs/blog/posts/2026-09-13-postgresql-partition-maintenance.md`
- **Category:** Libraries
- **Idea:** A tree of time-ordered partitions kept correct: one created ahead, one retired, one never touched.
...
Subject: A long parent bar with partition slabs standing beneath it in a row. At the right end one slab is being lowered into its place ahead of the others, still hovering above the grid. At the left end one slab has been lifted clear and set aside. Between them, one slab carries a small terracotta tag on its face and stays seated while the lifting bar passes over it without engaging.
