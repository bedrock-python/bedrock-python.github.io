...
- **Post:** `docs/blog/posts/2026-09-06-timeouts-are-not-deadlines.md`
- **Category:** Design
- **Idea:** A shared budget shrinks along a chain; identical per-hop timeouts do not.
...
Subject: Two arrangements stacked with clear space between them, joined at the right by one vertical boundary wall standing on the base plane. Upper: three service housings in a row over one long bar that runs straight through the boundary wall and continues well past it -- the whole protruding length beyond the wall is terracotta and is a substantial part of the bar. Lower: the same three housings, but the bar under them steps down shorter at each housing and its last remainder stops exactly at the wall.
