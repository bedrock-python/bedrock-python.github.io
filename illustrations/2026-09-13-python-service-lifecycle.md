# The Python service lifecycle: startup, health checks and shutdown

- **Post:** `docs/blog/posts/2026-09-13-python-service-lifecycle.md`
- **Category:** Design
- **Idea:** Stop being routed first, then drain, then finish, then exit.

Cover reused from the source article during the September 2026 consolidation.
...
Subject: A service housing whose body is divided into four segments standing on a base plane. Curved guide rails approach its first segment from the left carrying small blocks, and every one of them bends away before reaching it, diverting its blocks aside. Inside the later segments a few blocks are still being carried toward the far end. One block that reached the first segment rests against a raised closed plate there; that plate is terracotta.
