# Building HTTP and gRPC clients for production

- **Post:** `docs/blog/posts/2026-09-13-production-http-grpc-clients.md`
- **Category:** Design
- **Idea:** Keep the client; attach the policy beside it.
...
- an HTTP port: a wide rectangular socket plate

Subject: Two arrangements side by side. Left: an HTTP client cube sealed inside a heavy opaque shell with one small opening, the cube barely visible through it, the shell drawn in terracotta. Right: the same cube standing open and complete on the base plane, its own port plate exposed, with four thin policy plates docked around its outside, none of them covering its faces.
