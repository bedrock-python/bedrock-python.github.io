...
- **Post:** `docs/blog/posts/2026-09-07-circuit-breakers-should-be-per-origin.md`
- **Category:** Design
- **Idea:** One breaker per upstream, not one breaker for the client.
...
Subject: One service housing on the left joined to three service housings on the right by three separate straight rails, each interrupted midway by its own upright switch block in a slot. The top rail's switch stands lifted out of its slot, leaving a visible gap in that rail; the other two sit flush and their rails run unbroken. The raised switch is the only terracotta object.
