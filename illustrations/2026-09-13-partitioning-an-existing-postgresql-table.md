# Partitioning an existing PostgreSQL table

- **Post:** `docs/blog/posts/2026-09-13-partitioning-an-existing-postgresql-table.md`
- **Category:** Tutorials
- **Idea:** Adopt the old table as DEFAULT and drain it window by window while writes continue.
...
Subject: One large table slab on the left, open on one face and visibly emptying: a stream of rows leaves it and settles into partition slabs lined up under a parent bar on the right. Above the row, a chute descends from the top of the frame and keeps dropping new rows into the slabs, uninterrupted. The large table slab is terracotta and plainly emptier than the slabs it filled.
