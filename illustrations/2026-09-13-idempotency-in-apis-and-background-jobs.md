# Idempotency in APIs and background jobs

- **Post:** `docs/blog/posts/2026-09-13-idempotency-in-apis-and-background-jobs.md`
- **Category:** Design
- **Idea:** The case that matters is the retry that arrives while the first request is still running.
...
Subject: Two horizontal lanes overlapping in time, the second beginning before the first has finished, their overlap shown as a shaded band. Across the band sits a lock block seated in a key-shaped slot. The first lane passes beneath the lock and continues to a single effect disc; the second meets the lock's raised face and turns back. The lock is the terracotta object.
