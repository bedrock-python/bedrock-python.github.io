...
| 38 | Transport-independent errors: one domain error, HTTP and gRPC responses | Design | Lifecycle | review |
...
| 46 | Zero-dependency cores: why optional dependencies matter in infrastructure libraries | Design | Bedrock | review |
| 47 | Reliability is not `retry=3` | Design | Reliability | review |
| 48 | How I start a production-grade Python library in 2026 | Meta | Bedrock | review |
...
- Found while measuring, fixed and released as clientwright 0.2.2 (2026-09-07, #24 → #26, #25 → #28): an `attempt` ceiling escaped as a bare `TimeoutError` and was never retried, and `total` stopped at the response headers on httpx. The post's numbers are from a run against 0.2.2 and deadline-budget 0.1.3.
