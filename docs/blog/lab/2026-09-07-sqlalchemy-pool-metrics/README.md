...
metrics are sampled every half second and printed next to the application's own count of failed
checkouts.

```bash
uv venv --python 3.13 .venv
uv pip install --python .venv/bin/python "sqlalchemy-foundation-kit[settings,metrics]" asyncpg "testcontainers[postgres]"
