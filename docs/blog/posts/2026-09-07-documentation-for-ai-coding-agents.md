...
In 2024 I wrote documentation for developers. Somewhere in 2026 I noticed that a good share of the readers were not people. A coding assistant wiring one of my libraries into a service had invented a class that does not exist, called a synchronous function with `await`, and passed a session where the library wants an engine, and it had done all three with complete confidence, because the documentation it had read was written for someone who browses. This post is about what I changed: a page per library written for a model, the rules I put on it, the mechanics that hand it to a chat window in one click, and what it turned out to cost.
...
The lists run from fifteen to twenty rules per library. Writing them was the most useful documentation exercise I have done in years, for a reason that has nothing to do with models: to write the rule you have to know it, and I found two behaviours I had never written down anywhere while writing the first page.
...
The WRONG half is not invented. Each one is a thing a model actually produced against an earlier version of the docs, or a thing I watched a colleague do. That mixin was generated by an assistant that had read a page describing "the testcontainers integration" and inferred the class such an integration would have. Showing the wrong code next to the right code works better than any amount of prose about it, because a model pattern-matches on shape: it will recognise the shape it was about to produce and take the one beside it. There are 83 such pairs across the fourteen pages today.
