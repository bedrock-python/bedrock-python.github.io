# Libraries

Posts about the PyPI packages in the Bedrock Python ecosystem — design
notes and deep dives.

- **2026-09-07** — [One lifecycle for HTTP, gRPC, workers and cron jobs](../posts/2026-09-07-one-lifecycle-for-http-grpc-workers-and-cron.md)
- **2026-09-06** — [Managing PostgreSQL partitions, one failure at a time](../posts/2026-09-06-pg-partsmith.md)
- **2026-05-15** — [The Transactional Outbox pattern in Python: omni-box](../posts/2026-05-15-transactional-outbox-with-omni-box.md)
