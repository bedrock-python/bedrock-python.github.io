...
<figcaption><span class="bdr-diagram__eyebrow">ИДЕЯ В СХЕМЕ</span><strong>Устраняйте повтор эффекта, а не ID доставки</strong></figcaption>
...
---
flowchart LR
    accTitle: Устраняйте повтор эффекта, а не ID доставки
    accDescr: Две доставки могут представлять одну бизнес-операцию. Для дедупликации нужен её идентификатор; внешним эффектам по-прежнему нужна собственная защита от повторов при сбоях.
 A["Доставка A"] --> K["order-9.invoice"]
 B["Доставка B"] --> K
 K --> I["Проверка идемпотентности"] --> E["Отправить счёт"]
```

</div>
<p class="bdr-diagram__caption">Две доставки могут представлять одну бизнес-операцию. Для дедупликации нужен её идентификатор; внешним эффектам по-прежнему нужна собственная защита от повторов при сбоях.</p>
</figure>
<!-- /diagram:concept -->
