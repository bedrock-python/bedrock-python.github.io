...
<figcaption><span class="bdr-diagram__eyebrow">ИДЕЯ В СХЕМЕ</span><strong>Разделите смену структуры и перенос данных</strong></figcaption>
...
---
flowchart TD
    accTitle: Разделите смену структуры и перенос данных
    accDescr: Новый составной ключ и внешние ключи требуют отдельного плана. Подмена таблицы короткая; перенос данных из старой DEFAULT-партиции идёт отдельно, пачками.
 K["Подготовить составной ключ"] --> S["Подменить родителя; старую таблицу сделать DEFAULT"] --> P["Создать временные партиции"] --> D["Перенести строки пачками"] --> F["Восстановить составные внешние ключи"] --> E["Отсоединить пустую DEFAULT"]
```

</div>
<p class="bdr-diagram__caption">Новый составной ключ и внешние ключи требуют отдельного плана. Подмена таблицы короткая; перенос данных из старой DEFAULT-партиции идёт отдельно, пачками.</p>
</figure>
<!-- /diagram:concept -->
