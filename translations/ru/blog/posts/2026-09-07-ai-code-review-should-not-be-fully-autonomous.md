...
<figcaption><span class="bdr-diagram__eyebrow">ИДЕЯ В СХЕМЕ</span><strong>Два решения человека вокруг работы модели</strong></figcaption>
...
---
flowchart TD
    accTitle: Два решения человека вокруг работы модели
    accDescr: Человек задаёт контекст репозитория до анализа и утверждает замечания перед публикацией. Автоматическое ревью готовит основания для решения.
 H["Ревьюер задаёт задачу и ограничения"] --> B["brief"] --> D["dispatch"] --> P["polish"] --> A{"Ревьюер утвердил замечания?"}
 A -->|"Да"| O["post"]
 A -->|"Нужна доработка"| P
```

</div>
<p class="bdr-diagram__caption">Человек задаёт контекст репозитория до анализа и утверждает замечания перед публикацией. Автоматическое ревью готовит основания для решения.</p>
</figure>
<!-- /diagram:concept -->
