...
<figcaption><span class="bdr-diagram__eyebrow">ИДЕЯ В СХЕМЕ</span><strong>Проверяйте операцию, от которой зависит сервис</strong></figcaption>
...
---
flowchart TD
    accTitle: Проверяйте операцию, от которой зависит сервис
    accDescr: PING доказывает, что Redis отвечает на PING. Готовность должна учитывать нужную возможность: реплика или заполненный основной узел могут отвечать, но отклонять запись.
 P["PING"] --> Y{"Отвечает?"}
 Y -->|"Нет"| F["Зависимость недоступна"]
 Y -->|"Да"| W{"Сервису нужна запись?"}
 W -->|"Да"| R["Проверить запись + чтение"]
 W -->|"Нет"| G["Проверить нужное чтение"]
 R --> C["Определить готовность по возможности"]
 G --> C
```

</div>
<p class="bdr-diagram__caption">PING доказывает, что Redis отвечает на PING. Готовность должна учитывать нужную возможность: реплика или заполненный основной узел могут отвечать, но отклонять запись.</p>
</figure>
<!-- /diagram:concept -->
