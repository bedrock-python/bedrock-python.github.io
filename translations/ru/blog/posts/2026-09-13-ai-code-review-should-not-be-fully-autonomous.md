...
ИИ помогал мне находить ошибки в собственных merge request. Проблемы начались, когда я стал автоматизировать ревью кода коллег: полезные находки смешивались с неверными замечаниями и многословными комментариями. Этот опыт убедил меня, что проверку с ИИ лучше начинать с собственных изменений, до передачи MR команде.
...
Затем я стал использовать ИИ для ревью MR коллег. Полезные замечания вручную переносил в обсуждения, предварительно проверяя выводы и редактируя текст. Я понимал каждый опубликованный комментарий и мог объяснить его автору кода.
...
У коллеги процесс был полностью автономным. ИИ открывал обсуждения и отвечал в них без предварительной проверки человеком. Иногда от имени коллеги появлялось замечание, а затем автоматически отправлялся ответ в ту же ветку. Наблюдая за этим, я выделил три проблемы.
...
Длинные сгенерированные обсуждения затрудняют этот разговор. Сначала приходится искать конкретное замечание среди общих рассуждений. Если ответ на пояснение тоже отправляется автоматически, уже непонятно, участвует ли коллега в обсуждении.
...
Если поручить весь разбор агенту, реализация рискует остаться понятной только её автору. Пока он рядом, это может быть незаметно. Когда он уйдёт или окажется недоступен, остальным придётся восстанавливать контекст самостоятельно.
...
## Проверка с ИИ до передачи MR коллегам {#the-copilot-not-the-gatekeeper}

**Я предлагаю сделать проверку с ИИ обычным этапом подготовки изменений, за который отвечает автор.** Если агент способен найти проблему до обсуждения с командой, разработчик может запустить проверку сам. Он знает цель задачи, быстрее оценит замечания и исправит ошибки.

В рабочем процессе такая проверка занимает место рядом с pre-commit, линтерами и тестами. Но выводы модели требуют оценки: каждое замечание нужно проверить, а отсутствие замечаний не гарантирует корректность кода. MR может уже существовать как черновик — проверка должна предшествовать запросу ревью у коллег.
...
- **Момент проверки.** Для каких изменений нужна проверка с ИИ и после каких правок её следует повторить.
