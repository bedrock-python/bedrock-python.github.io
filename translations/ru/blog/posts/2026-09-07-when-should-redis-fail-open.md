...
<figcaption><span class="bdr-diagram__eyebrow">ИДЕЯ В СХЕМЕ</span><strong>Выбирать по цене повторной операции</strong></figcaption>
...
---
flowchart TD
    accTitle: Выбирать по цене повторной операции
    accDescr: Fail-open — бизнес-решение после ограниченного по времени сбоя Redis. Обход кеша может быть допустим; обход дедупликации платежа может повторить списание, поэтому приложение должно явно выбрать политику.
    R["Таймаут / сбой Redis"] --> Q{"Роль Redis?"}
    Q -->|"Необязательный кеш"| C["Получить из основного источника"]
    Q -->|"Ограничение запросов"| L["Явный обход + метрика"]
    Q -->|"Идемпотентность"| I{"Повтор безопасен?"}
    I -->|"Да"| O["Обойти, если разрешено"]
    I -->|"Нет"| F["Отказать или отложить операцию"]
```

</div>
<p class="bdr-diagram__caption">Fail-open — бизнес-решение после ограниченного по времени сбоя Redis. Обход кеша может быть допустим; обход дедупликации платежа может повторить списание, поэтому приложение должно явно выбрать политику.</p>
</figure>
<!-- /diagram:concept -->
